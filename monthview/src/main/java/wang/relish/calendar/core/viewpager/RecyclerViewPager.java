package wang.relish.calendar.core.viewpager;
...
public class RecyclerViewPager extends RecyclerView {
    ...
    public boolean onTouchEvent(MotionEvent e) {
        ...
        if (e.getAction() == MotionEvent.ACTION_MOVE) {
            if (mCurView != null) {
                int halfWidth = mCurView.getWidth() / 2;
                mMaxLeftWhenDragging = Math.max(mCurView.getLeft() - halfWidth, mMaxLeftWhenDragging);
                mMaxTopWhenDragging = Math.max(mCurView.getTop(), mMaxTopWhenDragging);
                mMinLeftWhenDragging = Math.min(mCurView.getLeft() + halfWidth, mMinLeftWhenDragging);
